...
async def aexec(code, Royalboyamit, message):
    exec(
        "async def __aexec(client, message): "
        + "".join(f"\n {a}" for a in code.split("\n"))
    )
    return await locals()["__aexec"](Royalboyamit, message)
...
async def executor(Royalboyamit, message):
    ...
    try:
        await aexec(cmd, Royalboyamit, message)
    except Exception:
...
@Royalboyamit.on_callback_query(filters.regex(r"runtime"))
